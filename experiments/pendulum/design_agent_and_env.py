...
def design_agent_and_env(FLAGS):
    ...
    timesteps_per_action = FLAGS.timesteps_per_action  # Provide the number of time steps per atomic action.
    ...
    # Provide initial state space consisting of the ranges for all joint angles and velocities.  In the inverted
    # pendulum task, we randomly sample from the below initial joint position and joint velocity ranges.  These
    # values are then converted to the actual state space, which is [cos(pendulum angle), sin(pendulum angle),
    # pendulum velocity].
    ...
    # In the inverted pendulum environment, the end goal will be the desired joint angle and joint velocity for the
    # pendulum.
    ...
    # agent should be given the sparse reward.  It is also used for Hindsight Experience Replay to determine which
    # end goal was achieved after a sequence of actions.
    ...
    project_state_to_end_goal = lambda sim, state: np.array(
        [bound_angle(sim.data.qpos[0]), 15 if state[2] > 15 else -15 if state[2] < -15 else state[2]])
    ...
    # the inverted pendulum task, the subgoal space is the concatenation pendulum position and velocity.  This is
    # slightly different than the state space, which is [cos(pendulum angle), sin(pendulum angle), pendulum velocity].
    ...
    # subgoal just equals penalty.  For instance in this 3-level pendulum implementation, if a level proposes a
    # subgoal and misses it, the Q target value for this action would be -10.  To incorporate the next state in the
    # penalty, go to the "penalize_subgoal" method in the "layer.py" file.
    agent_params["subgoal_penalty"] = -FLAGS.time_scale

    # Define exploration noise that is added to both subgoal actions and atomic actions.  Noise added is Gaussian N(
    # 0, noise_percentage * action_dim_range)
    ...
    # Hyperparameter below indicates number of exploration episodes.  Testing occurs for 100 episodes.  To change
